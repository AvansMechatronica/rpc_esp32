...
// MCP2515v01.cpp
//
// inspired by & copied from https://github.com/autowp/arduino-mcp2515
//
// RASM: custom SPI control added for use with NodeMCU32s platform
// RASM: custom I2C/SPI interface added
//
...
///////////////////////////////////////////////////////////////////////////////
// bridge device from I2C to SPI to access the MCP2515 SPI device from the I2C
// bus

# if (I2CSPI_BRIDGE_USED == 1)
static SC18IS606 spiBridge(-1, 0, 0, 0);	// -1: do not use reset pin
#endif

///////////////////////////////////////////////////////////////////////////////
// SPI settings for quadrature MCP2515 transactions

# if (I2CSPI_BRIDGE_USED == 0)
static SPISettings g_MCP2515SPISettings(SPI_MCP2515_SPEED, SPI_MSBFIRST, SPI_MODE0);
#endif

///////////////////////////////////////////////////////////////////////////////
// MCP2515 definitions and data
...
///////////////////////////////////////////////////////////////////////////////
// MCP2515::MCP2515(const uint8_t i2cAddress, const uint8_t spiSlaveNum)
// RASM constructor added: MCP2515 accessed by I2C/SPI bridge

# if (I2CSPI_BRIDGE_USED == 1)	// constructor for I2C bridge use
MCP2515::MCP2515(const uint8_t i2cAddress, const uint8_t spiSlaveNum)
{
	i2cBridgeAddress = i2cAddress;
	spiSlaveAddress  = spiSlaveNum;
}
#else
MCP2515::MCP2515(uint8_t spiDeviceNumber)	// constructor for standard SPI use
{
	this->spiDeviceNumber = spiDeviceNumber;
}
#endif


///////////////////////////////////////////////////////////////////////////////
// MCP2515::MCP2515(const uint8_t i2cAddress)
// RASM function begin added: hiero 14-12

#if (I2CSPI_BRIDGE_USED == 1)
MCP2515::ERROR MCP2515::begin(void)
{
	// spiBridge.begin() not required, handled by system supplied i2c_Init()!!
	// spiBridge.reset() not used, hardware reset on power up

	i2c_Init();

	if (i2cBridgeAddress == 0) // I2C bridge address not set in constructor!
	{
		return ERROR_FAIL;
	}

	bool lsbFirst = false;	// MSB is first!!
	spiBridge.configureSPI(lsbFirst, SC18IS606_SPIMODE_0, SC18IS606_SPICLK_1875_kHz);
...
void MCP2515::startSPI() 
{
	spi_BeginTransaction(g_MCP2515SPISettings);
	spi_SelectDevice(spiDeviceNumber);
}
#endif
...
MCP2515::ERROR MCP2515::reset(void)
{
#if (I2CSPI_BRIDGE_USED == 0)
	startSPI();
	spi_TransferByte(INSTRUCTION_RESET);
	endSPI();
#else
	uint8_t txData[2];
	uint8_t rcData[2];
	...
	txData[0] = INSTRUCTION_RESET;
	spiBridge.spiTransfer(spiSlaveAddress, txData, 1, rcData);
#endif
...
uint8_t MCP2515::readRegister(const REGISTER reg)
{
...
#if (I2CSPI_BRIDGE_USED == 0)
	startSPI();
	spi_TransferByte(INSTRUCTION_READ);
	...
	spiBridge.spiTransfer(spiSlaveAddress, txData, 3, rcData);
	val = rcData[2];
#endif
...
void MCP2515::readRegisters(const REGISTER reg, uint8_t values[], const uint8_t n)
{
	...
	{
		values[i] = spi_TransferByte(0x00);
	}
	endSPI();
#else
	uint8_t txData[20];
	...
	spiBridge.spiTransfer(spiSlaveAddress, txData, n + 2, rcData);
...
#if (I2CSPI_BRIDGE_USED == 0)
	startSPI();
	spi_TransferByte(INSTRUCTION_WRITE);
	spi_TransferByte(reg);
...
void MCP2515::modifyRegister(const REGISTER reg, const uint8_t mask, const uint8_t data)
{
#if (I2CSPI_BRIDGE_USED == 0)	
    startSPI();
	spi_TransferByte(INSTRUCTION_BITMOD);
	spi_TransferByte(reg);
	spi_TransferByte(mask);
	...
	spiBridge.spiTransfer(spiSlaveAddress, txData, 4, rcData);
#endif		
}
...
uint8_t MCP2515::getStatus(void)
{
...
#if (I2CSPI_BRIDGE_USED == 0)
	startSPI();
	spi_TransferByte(INSTRUCTION_READ_STATUS);
	...
	txData[0] = INSTRUCTION_READ_STATUS;

	spiBridge.spiTransfer(spiSlaveAddress, txData, 2, rcData);
	status = rcData[1];
#endif
...
///////////////////////////////////////////////////////////////////////////////
// void MCP2515::printCANError(ERROR errorCode)
...
void MCP2515::printCANError(ERROR errorCode)
{
	...
	case ERROR_FAIL:
		SerialPrintf("> CAN error: FAIL\n");
		break;

	case ERROR_ALLTXBUSY:
		SerialPrintf("> CAN error: ALLTXBUSY\n");
		break;

	case ERROR_FAILINIT:
		SerialPrintf("> CAN error: FAILINIT\n");
		break;

	case ERROR_FAILTX:
		SerialPrintf("> CAN error: FAILTX\n");
		break;

	case ERROR_NOMSG:
		SerialPrintf("> CAN error: NOMSG\n");
		break;

	default:
		SerialPrintf("> CAN error: UNDEFINED (%d)\n", errorCode);
		break;
	}
}
